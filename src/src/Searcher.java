...
public class Searcher {
    ...
    private String[] getSizes(ArrayList<File> documents) throws IOException {
        ...
        for (int i = 0; i<documents.size();i++){
            sizes[i]=String.valueOf(documents.get(i).length())+" bytes";
        }
        return sizes;
    }
    ...
    private String getContext(File document, int lineNumber, int linePos){
        ...
        try {
            for (int i = 0; i < 3; i++) {
                context = content[lineNumber + Math.floorDiv(linePos - i, content.length)][Math.floorMod(linePos - i, content.length)] + context;
            }
